...
const ConsultExpertForm: React.FC<ConsultExpertFormProps> = ({
  serviceName,
  onSubmit,
}) => {
  // State for form data
  ...
  const handleGetStarted = (e: FormEvent) => {
    ...
    const sanitizedData = Object.fromEntries(
      Object.entries({ ...formData, plan: serviceName }).filter(
        ([_, val]) => !!val
      )
    ) as Partial<FormData>;
  ...
  return (
    <div className="w-full max-w-md bg-white border border-gray-200 py-6 px-6">
      <div className="text-[20px] text-black font-bold mb-4 text-center">
      ...
      <form className="flex flex-col gap-4" id="service-form">
        <div className="grid grid-cols-2 gap-1">
          <div>
            <label htmlFor="email" className="block mb-1 font-medium">First Name</label>
            <input
              type="text"
              id="firstName"
              ...
              required
              className="w-full h-12 border border-gray-300 rounded-md px-3 py-2 
        ...
        <div>
          ...
          />
          {error.email && (
            <div className="text-red-600 text-sm">* Email is required</div>
          )}
        </div>
        ...
        <div>
          ...
          />
          {error.city && (
            <div className="text-red-600 text-sm">* City is required</div>
          )}
        </div>
        ...
        <div>
          ...
          />
          {error.pincode && (
            <div className="text-red-600 text-sm">* Pincode is required</div>
          )}
        </div>
